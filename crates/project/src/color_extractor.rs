...
lazy_static! {
    static ref RELAXED_HEX_REGEX: Regex = RegexBuilder::new(HEX)
        .case_insensitive(false)
        .build()
        .unwrap();
    static ref STRICT_HEX_REGEX: Regex = RegexBuilder::new(&format!("^{HEX}$"))
        .case_insensitive(true)
        .build()
        .unwrap();
    static ref RELAXED_RGB_OR_HSL_REGEX: Regex = RegexBuilder::new(RGB_OR_HSL)
        .case_insensitive(false)
        .build()
        .unwrap();
    static ref STRICT_RGB_OR_HSL_REGEX: Regex = RegexBuilder::new(&format!("^{RGB_OR_HSL}$"))
        .case_insensitive(true)
        .build()
        .unwrap();
}
...
mod tests {
    ...
    lazy_static! {
        static ref COLOR_TABLE: Vec<(&'static str, Option<u32>)> = vec![
          // -- Invalid --
          // Invalid hex
          ("f0f", None),
          ("#fof", None),
